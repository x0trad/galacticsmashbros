...
const source=JSON.parse(fs.readFileSync(path.join(root,'monkey-green-cap.sprite.json'),'utf8'));
...
const sheet=Buffer.alloc(size*4*size*10*4),atlas={frames:{},meta:{image:'monkey-sheet.png',size:{w:768,h:1920},frameSize:{w:192,h:192},scale:3,frameTags:[]}};
let count=0;
for(const [ai,name] of order.entries()){
 ...
 const [action,dir]=name.split('-'),folder=`${dir.toUpperCase()}_${action.toUpperCase()}_Monkey`;fs.mkdirSync(path.join(root,'frames',folder),{recursive:true});
 for(const [fi,f] of a.frames.entries()){
  ...
  const data=pixels(f.rows),filename=`monkey-${name}-${fi+1}.png`;
