...
export const wallets=sqliteTable('wallets',{playerId:text('player_id').primaryKey().references(()=>players.id),address:text('address').notNull().unique(),chainId:integer('chain_id').notNull(),linkedAt:integer('linked_at').notNull()});
export const walletChallenges=sqliteTable('wallet_challenges',{playerId:text('player_id').primaryKey().references(()=>players.id),nonce:text('nonce').notNull(),address:text('address').notNull(),message:text('message').notNull(),expiresAt:integer('expires_at').notNull(),consumed:integer('consumed').notNull().default(0)});
